...
Geschwindigkeit $\alpha$ der Lösung $u$ an der jeweiligen Stelle. Das Verhalten
der Burgersgleichung ist also ähnlich zur linearen Advektion.
:::
::::
...
Anfangsbedingung $u(x,t=0) = e^{-40 (x-\frac{1}{2})^2}$ und eine periodische
...
**Lineare Diffusion**
$$
\pp{u}{t} = \beta \pp{^2u}{x^2} = 0 \; \text{mit} \; \beta = 1 \frac{m^2}{s}
$$
```{python}
#| echo: false
#| warning: false
...
$$
\pp{u}{t} + \alpha \pp{u}{x} = 0 \; \text{mit} \; \alpha = 1 \frac{m}{s}
$$
```{python}
#| echo: false
#| warning: false
